...
mod integration_tests {
    ...
    async fn test_full_event_flow() {
        ...
        // Start file watcher
        let mut watcher = FileWatcherPlugin::new("test", vec![temp_dir.clone()])
            .with_pattern("*.txt");
        watcher.start(sender).await.expect("Failed to start watcher");
        ...
        let event = timeout.await.expect("Timeout waiting for event")
